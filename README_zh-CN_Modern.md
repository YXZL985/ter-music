...
### 九 构建脚本使用
本项目提供5种构建脚本，用于生成不同格式的可执行文件。

#### 1. AppImage构建脚本
`build-appimage.sh` 脚本可直接从源码构建AppImage格式文件（也可从已有RPM包转换），兼容各类Linux发行版。
**使用方法**：
```bash
# 指定版本号从源码构建（推荐）
...
**输出位置**：AppImage包会生成在`build/appimage/<arch>/`目录下

#### 2. 便携包构建脚本
`build-portable.sh` 脚本可直接从源码构建便携压缩包（也可从已有RPM包转换），包含所有必需的依赖库。
**使用方法**：
```bash
# 指定版本号从源码构建（推荐）
...
- x86_64：Intel/AMD 64位
- aarch64：ARM 64位
- loong64：龙芯（包含新世界与旧世界）
- loongarch64：龙芯旧世界
- sw64：申威
- mips64：MIPS 64位

**输出位置**：便携包会生成在`build/portable/<arch>/`目录下
**注意**：该脚本现已支持直接从源码构建。也可先运行`build-rpm.sh`生成RPM包，再通过`-r`选项指定RPM包路径进行转换。

#### 3. RPM包构建脚本
`build-rpm.sh` 脚本可直接从源码构建标准RPM包，适用于Fedora/RHEL等基于RPM的发行版。
**使用方法**：
```bash
# 自动检测版本和架构构建RPM包
./build-rpm.sh
...
**输出位置**：RPM包会生成在`build/rpm/<arch>/`目录下
**安装方法**：
```bash
...
#### 4. 玲珑包（Linyaps）构建脚本
`build-linyaps.sh` 脚本可直接从源码构建玲珑包，适用于deepin/UOS等使用玲珑包管理的系统。
**使用方法**：
```bash
# 自动检测版本和架构构建玲珑包
./build-linyaps.sh
...
**输出位置**：UAB包和layer文件会生成在`build/linyaps/<arch>/`目录下
**安装运行**：
```bash
# 使用ll-cli安装
...
#### 5. DEB包构建脚本
`build-deb.sh` 脚本可直接从源码构建标准Debian/Ubuntu DEB包，适用于Debian、Ubuntu、Linux Mint、deepin等基于Debian的发行版。
**使用方法**：
```bash
# 指定版本号从源码构建（推荐）
...
**输出位置**：DEB包会生成在`build/deb/<arch>/`目录下
**安装方法**：
```bash
...
#### 6. Arch Linux PKGBUILD构建脚本
`PKGBUILD` 脚本可直接从源码构建标准Arch Linux包，适用于Arch Linux及其衍生发行版。
**使用方法**：
```bash
# 从AUR克隆PKGBUILD文件
git clone https://aur.archlinux.org/ter-music-cn.git
...
- **@guanzi008** - 深入优化多项功能：完善DEB打包、修复UTF-8输入问题、优化设置引导、支持鼠标交互、规整歌单管理、优化目录排序、修复播放中断问题、提升性能、美化频谱显示、优化中文界面展示
