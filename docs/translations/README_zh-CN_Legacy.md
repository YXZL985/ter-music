...
- **playlist.c**：乐目之加载，目录之搜检，元数据之读取（FFmpeg + 原生 APEv2 标签）
- **ape_tag.c**：原生 APEv2 标签解析器，以增元数据之提取
- **progress.c**：音程之追蹑，跳转之节度
- **lyrics.c**：歌辞之加载、解析、同步显明之制
- **menu_views.c**：众视之管摄，节度、往迹、珍存、曲帙之理
- **remote.c**：远程播乐之能（SMB/SFTP/FTP/WebDAV诸约）
...
- 可试于设界面（按`F2`入设）增"输出时延"之值，或径修节文`~/.config/ter-music/config.json`中`audio_latency_ms`之域
- 时延之域自20至250千分秒，每增10而验，至音声复其常
- 若增时延而未解，可察波氏之制或更新音声驱策
