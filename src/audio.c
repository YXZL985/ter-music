...
void *play_audio_thread(void *arg) {
    ...
    while (g_play_thread_running) {
        ...
        if (g_seek_request && g_play_thread_running && fmt_ctx && codec_ctx) {
            ...
            // BUGFIX 2026.03.26: 使用正确的 audio stream index 和 AVSEEK_FLAG_BACKWARD
            // 指定 audio_stream_index 确保跳转到正确的流
            // AVSEEK_FLAG_BACKWARD 确保跳转到关键帧之前，定位更准确
...
void seek_audio(int position) {
    ...
    update_progress_bar();
    render_controls();
    // BUGFIX 2026.03.26: seek 后立即刷新歌词，保证同步
